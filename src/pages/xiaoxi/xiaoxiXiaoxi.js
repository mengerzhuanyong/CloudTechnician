...
export default class XiaoxiXiaoxi extends Component {
    ...
    toggleContent = (msg_id, content) => {
        let url = NetApi.read_msg;
        let data = {
            msg_id: msg_id,
        ...
        Services.Post(url, data)
            .then( result => {
                // console.log(result);
                this.toWebview(content, 'XiaoxiDetail');
            })
            .catch( error => {
                // consoleLog('链接服务器出错，请稍后重试', error);
            })
    ...
    renderCompanyItem = ({item}) => {
        ...
        return (
            <TouchableOpacity onPress={() => {this.toggleContent(item.id, content)}} style={[styles.gonggaoList, GlobalStyle.flexRowStart]}>
                {item.is_read === 0 ? 
                    <View style={[styles.circleIcon, styles.redIcon]}></View>
                    :
