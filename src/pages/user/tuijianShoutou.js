...
export default class TuijianShoutou extends Component {
    constructor(props) {
        super(props);
    ...
    loadNetData = () => {
        let url = NetApi.get_team_first_buy_list;
        let data = {
                ...
                <TouchableOpacity onPress={() => {RouterHelper.navigate('', 'OrderRenshengouDetail', item.order_id)}} style={styles.orderTop}>
                    <View style={[styles.orderInfo, GlobalStyle.flexRowBetween]}>
                        ...
                        <View style={[styles.orderdate, GlobalStyle.flexRowBetween]}>
                            <Text style={styles.orderTime}>首投日期: {item.confirm_day}</Text>
                            <Text style={styles.orderMoney}><Text style={{fontSize: 13}}>首投金额: </Text>{item.money}</Text>
                        </View>
                </TouchableOpacity>
            </View>
        )
